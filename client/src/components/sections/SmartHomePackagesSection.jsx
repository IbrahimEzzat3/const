      ...
      >
        ...
        {pkg?.notInclude?.map((notInclude, idx) => (
          <li key={idx} className="flex items-center justify-start">
            ...
            <span className="mr-2 text-accent-gold">{notInclude}</span>
          </li>
        ))}
      </ul>
    ...
    <section id="packages" className="py-16 mt-16 bg-[#F5EFE6] ">
      <div className="w-full px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-16 max-w-5xl mx-auto">
          {packages.slice(0, 3).map((pkg, index) => renderPackage(pkg, index))}
        </div>

        {/* الخط الفاصل مع النص */}
        <header className="text-center mb-12 mt-12">
          <h2 className="text-3xl md:text-4xl font-bold text-accent-teal mb-4">
            {t("sections.smartHomePackages.title2")}
          </h2>
          <div className="w-24 h-1 bg-accent-gold mx-auto"></div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-16 max-w-5xl mx-auto">
          {packages
            .slice(3, 6)
            .map((pkg, index) => renderPackage(pkg, index + 3))}
        </div>
      </div>
    </section>
  );
